...
- **卡片/图片渲染依赖 headless Chrome**：金句卡、今日判断日卡、小红书导出、素材盘热点板都走
  「HTML→Chrome 截图」。Mac 本机用系统 Chrome；VPS 用**官方 Google Chrome**（deb 包），
  `.env` 设 `CHROME=/usr/bin/google-chrome`。⚠️ 别用 Ubuntu 的 `apt install chromium`——
  它实际是 Snap 包，沙箱限制与 /srv 下的渲染路径冲突（2026-07-19 实测，生产为此改装官方
  Chrome 150）。⚠️ 还必须 `apt install fonts-noto-cjk`：服务器默认无中文字体，Chrome 会把
  中文全部渲染成空白（英文数字正常、中文消失即此症状）。缺 Chrome 时这些出口 503/跳过，
  其余功能不受影响。
...
| `CHROME` | headless Chrome 路径 | 默认 macOS 系统 Chrome；VPS 设 `/usr/bin/google-chrome`（勿用 Snap chromium，见「部署形态」） |
...
- **LLM 调用日志**：全部 11 个 AI 角色（改写/重评/嘉宾/金句/聚类/主编/延伸阅读/问答/终审/
  小红书文案/海报理由）的每次调用都追加一行到 `blog/data/llm_calls.jsonl`（caller/model/耗时/tokens/错误），
...
**分层纪律**：对内文字卡片透明（源名+数字都在）且**零加工**；对外分享图收敛——
**绝不出现聚合源名与原始热度量纲**（两套数字并排即可指纹识别数据源），
用「今日最热/还在热议」定性标签 + 「综合公开讨论热度」中性脚注，有测试锁死该契约。
对外海报是素材盘唯一的 LLM 例外（第 11 个角色「海报理由」）：把聚合源的原始简介
改写成一句给读者的「为什么值得看」，失败/形状不符回退原始摘要（原始摘要有句读
截断兜底），海报绝不因此空窗。证据链分层：条目与当日简报主题重合时**优先复用简报
里已核验的 why_ranked**（不经 LLM）；现编的理由未经原文核验，因此图片脚注固定带
「速览未逐条核验」诚实标注。
