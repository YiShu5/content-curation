...
- **卡片/图片渲染依赖 headless Chrome**：金句卡、今日判断日卡、小红书导出都走「HTML→Chrome 截图」。
  Mac 本机用系统 Chrome；VPS 需 `apt install chromium` 并让 `text_card.CHROME`
  指到它（或设环境变量）。缺 Chrome 时这些出口返回 503，博客其余功能不受影响。
...
PNG 图组（封面 + 每主题一张，纸墨编辑部风）+ DeepSeek 生成的 `文案.txt`（标题/正文/tag，
失败不影响图片），打包 zip。依赖 Chrome（见「部署形态」）。文案 agent 是第 10 个 AI 角色。
