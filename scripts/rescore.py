...
total = 三者之和。verdict 按 total：{verdict_bands}。
...
def rescore(title, guests, body):
    content = (PROMPT
               .replace("{rubric_insight}", _rubric_line("insight"))
               .replace("{rubric_source}", _rubric_line("source"))
               .replace("{rubric_storytelling}", _rubric_line("storytelling"))
               .replace("{verdict_bands}", _verdict_bands())
